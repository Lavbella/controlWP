...
with aba_editar:
    ...
    with st.spinner("A carregar artigos recentes..."):
        try:
            #res_posts = session.get(f"{API_URL}/wp/v2/posts", params={"per_page": 50, "status": "publish"})

            # Enviamos o status como uma lista de parâmetros repetidos (padrão robusto HTTP)
            res_posts = session.get(
            ...
            if res_posts.status_code == 200:
                ...
                if lista_posts:
                    ...
                    edit_conteudo = st.text_area("Editar Conteúdo", value=post_dados['content']['rendered'], height=400)
